...
window.submitForm = async function () {
  ...
  if (!addressInput) {
    status.innerHTML =
      '<span class="error">Please enter a pickup address.</span>';
    return;
  }

  if (!selectedAddress) {
    status.innerHTML =
      '<span class="error">Please select an address from the dropdown.</span>';
    return;
  }
  if (!firstName || !lastName || !phone || !email || !description || !date) {
    status.innerHTML =
